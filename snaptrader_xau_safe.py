...
symbol = st.sidebar.selectbox("اختر الأصل المالي", ["XAUUSD=X", "BTC-USD", "ETH-USD", "SOL-USD", "AAPL", "TSLA"])
...
if data.empty:
    st.error("⚠️ لا توجد بيانات متاحة في الفترة الزمنية المحددة.")
    st.stop()

if "Close" not in data.columns:
    st.error("⚠️ لا توجد بيانات إغلاق (Close) في الأصل المحدد.")
    st.stop()

if data["Close"].isnull().all():
    st.error("⚠️ جميع قيم الإغلاق فارغة. يرجى اختيار فترة زمنية أخرى.")
    st.stop()
